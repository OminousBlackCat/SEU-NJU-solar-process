...
本py文件内部包含了所有在对太阳图像预处理时使用的处理函数与工具函数
所有与图像处理相关的算法代码都在本文件内
@author: seu_lcl
@editor: seu_wxy
"""
...
def getBin(imgData):
    H, W = imgData.shape
    util.log(H, W, height_Ha + height_Fe)
...
def cal_center_mean(Input_np):
    ...
    util.log("HA_U 与 Ha_L:", Ha_U, Ha_L)
    util.log("Fe_U 与 Fe_L:", Fe_U, Fe_L)

    mean_K = (FE_lineCore - HA_lineCore) / (Fe_mean - Ha_mean)
    mean_b = FE_lineCore - Fe_mean * mean_K
    util.log("线心像元为: ", Fe_mean, Ha_mean)
    util.log("结果为:", mean_K, mean_b)
